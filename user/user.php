...
try {
    ...
    $stmt = $pdo->query("
      SELECT w.logged_at, w.reps, w.weight, e.name
      FROM workout_logs w
    ...
    foreach($rows as $r){
        ...
        if(!isset($workoutDays[$dateKey])){
            $workoutDays[$dateKey] = [
                ...
                'total_sets'=>1,
                'total_volume'=>((int)$r['reps'] * (float)$r['weight']),
                'exercises'=>[ ['name'=>$r['name'],'sets'=>1,'reps'=>(int)$r['reps'],'max_weight'=>(float)$r['weight']] ]
            ];
        } else {
            $workoutDays[$dateKey]['total_sets'] += 1;
            $workoutDays[$dateKey]['total_volume'] += ((int)$r['reps'] * (float)$r['weight']);
            $workoutDays[$dateKey]['exercises'][] = ['name'=>$r['name'],'sets'=>1,'reps'=>(int)$r['reps'],'max_weight'=>(float)$r['weight']];
        }
    }
          ...
          <div class="history-workout">
            ...
            <div class="workout-details" style="width:100%;">
              ...
              <div class="workout-stats-mini">
                ...
                <span><i class="fas fa-weight-hanging"></i> <?php echo number_format((float)$wDay['total_volume'],1); ?> kg volume</span>
              </div>
              ...
              <div style="display:none;margin-top:10px;">
                <?php foreach($wDay['exercises'] as $ex): ?>
                  <div style="display:flex;justify-content:space-between;gap:10px;padding:6px 0;border-bottom:1px solid rgba(255,255,255,0.08);">
                    ...
                    <span><?php echo (int)$ex['sets'];?> sets, <?php echo (int)$ex['reps'];?> reps, max <?php echo number_format((float)$ex['max_weight'],1); ?> kg</span>
                  </div>
                <?php endforeach; ?>
              </div>
